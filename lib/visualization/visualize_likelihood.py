...
def draw_likelihood_circles_on_image(image,likelihoods,iteration,station,observation):
    ...
    for name,(x,y) in dic.items():
        r=likelihoods[j]*100
        draw.ellipse((x-r, y-r, x+r, y+r), fill=(255, 0, 0,100), outline=(255, 0, 0))
        j+=1

    # draws a blue circle at the actual station
    x,y=dic[station.name]
    r=30
    draw.ellipse((x-r, y-r, x+r, y+r), fill=(0, 0, 255,100), outline=(0, 0, 255))
...
def show_likelihoods(image_file,likelihoods,stations,observations,output_dir=None):
    ...
    for i in range(n):
        cur_image=images[i]
        draw_likelihood_circles_on_image(cur_image,likelihoods[i],i+1,stations[i],observations[i])
